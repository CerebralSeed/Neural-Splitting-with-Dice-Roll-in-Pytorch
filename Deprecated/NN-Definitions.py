...
def roll_dice(model_class, criterion, batches, rolls, trainloader):
    ...
    best_loss = 0
    model = model_class
    model.to(device) #comment out for CPU
    for epoch in range(rolls):  # test "roll" number of randomized parameter vectors
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            inputs, labels = data[0].to(device), data[1].to(device) #'inputs, labels = data' <---- if on CPU change to this
...
def test_data(model, testloader):
    with torch.no_grad():  # check accuracy on test set
        ...
        for tdata in testloader:
            timages, tlabels = tdata[0].to(device), tdata[1].to(device) #'inputs, labels = data' <---- if on CPU change to
