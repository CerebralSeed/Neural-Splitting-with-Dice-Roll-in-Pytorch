...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(300, 25, 5)
        self.fc1 = nn.Linear(25 * 5 * 5, 15)
...
'''
def hist_gradients(accumulated_grad):
    grad_cpu=accumulated_grad[1].cpu()
    _ = plt.hist(grad_cpu.numpy(), bins='auto')  # arguments are passed to np.histogram
    plt.title("Histogram with 'auto' bins")
    print(torch.mean(grad_cpu))
    plt.show()
'''
criterion = nn.CrossEntropyLoss()

net = roll_dice(Net(), criterion, 10, rolls=30,
                trainloader=trainloader)  # get best random starting parameters - avoid bad starts
...
for epoch in range(50):  # loop over the dataset multiple times
    ...
    #hist_gradients(accumulated_grad)
